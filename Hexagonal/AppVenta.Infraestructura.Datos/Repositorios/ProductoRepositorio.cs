    ...
    public class ProductoRepositorio : IRepositorioBase<Producto, Guid>
    {
        ...
        public void Editar(Producto entidad)
        {
            var productoSeleccionado
                = db.Productos.Where(c => c.productoId == entidad.productoId).FirstOrDefault();
            if (productoSeleccionado != null)
            {
                ...
                db.Entry(productoSeleccionado).State =
        ...
        {
            var productoSeleccionado
                = db.Productos.Where(c => c.productoId == entidadId).FirstOrDefault();
            if (productoSeleccionado != null)
            {
        ...
        {
            return db.Productos.Where(x => x.productoId.Equals(entidadID)).FirstOrDefault();
        }
    }
}
